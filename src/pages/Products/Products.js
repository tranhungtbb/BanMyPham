...
export default class Products extends Component {
    ...
    async componentDidMount() {
        ...
        await Call('api/getListProductByCategory?menuAlias=' + alias, 'GET', headers).then(res => {
            this.setState({
                products: res.data
            })
        })

        await Call('api/getTypeSorts', 'GET', headers).then(res => {
            this.setState({
                typesort: res.data
            })
        })
    ...
    async sortBy(typeSort, pageIndex) {
        const value=qs.parse(this.props.location.search);
        const alias=value.alias;
        let headers = new Headers();
        headers.append('Access-Control-Allow-Origin', '*');
        ...
        await Call('api/getListProductByCategory?menuAlias=' + alias
            +'&&typeSort=' + typeSort
            +'&&pageNumber=' + pageIndex
            , 'GET', headers).then(res => {
            this.setState({
                products: res.data
            })
        })
    ...
    render() {
        const {active, products , typesort} = this.state;
        return (
                                                    ...
                                                    <div className="tickid-sort-by-options">
                                                        {
                                                            typesort.map((item, index)=>{
                                                                if(index < 3){
                                                                    return(
                                                                        <a className={classNames('tickid-sort-by-options__option',
                                                                            {'tickid-sort-by-options__option--selected' : active === index })}
                                                                            onClick={()=> this.sortBy(item.typeSort,1)}
                                                                        > {item.titleSort}</a>
                                                                ...
                                                                <div className="select-with-status__dropdown tickid-modal__transition-enter-done">
                                                                    {
                                                                        typesort.map((item, index)=>{
                                                                            if(index >= 3){
                                                                                return(
                                                                                    <a className="select-with-status__dropdown-item select-with-status__dropdown-item--with-tick js-option"
                                                                                    onClick={()=> this.sortBy(item.typeSort,1)}
                                                                                    >
                                                                                        {item.titleSort}
                                                                                    </a>
